...
def train_model_with_val(model, train_loader, val_loader, criterion, optimizer, device, num_epochs=10, early_stopper=None):
    ...
    for epoch in range(num_epochs):
        ...
        mlflow.log_metric("train/loss", round(epoch_loss,3), step=epoch)
        mlflow.log_metric("train/acc", round(epoch_acc,3), step=epoch)
        mlflow.log_metric("val/loss", round(val_epoch_loss,3), step=epoch)
        mlflow.log_metric("val/acc", round(val_epoch_acc,3), step=epoch)
